...
export default class BoundingRect {
   ...
   * @param other 另一个包围盒
   */
  public extend(other: BoundingRect) {
  ...
  public transform(m: Matrix) {
    ...
    let minX: number, minY: number, maxX: number, maxY: number, x: number, y: number;
   ...
   * 检测两个包围盒区域是否相交
   */
  public intersect(x: BoundingRect | number = 0, y: number = 0, w: number = 0, h: number = 0) {
